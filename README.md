...
행성 규모 룰 위반을 **감시만** 하고, **ConsciousnessSignal(의식 경고)** 만 내보내는 독립 패키지.  
...
| | |
|---|---|
| **한 줄** | 행성 규모 제약을 **측정만** 하고, **불변 신호**로만 전달하는 **진단 엔진**. |
| **입력** | `tick`, (선택) `deep_engine`, (선택) `world_snapshot`. |
| **출력** | `List[ConsciousnessSignal]` — 위반 없으면 `[QUIET]`, 복합 위반 시 여러 신호. |
| **레이어** | **50_DIAGNOSTIC_LAYER** — 거시 감시·진단 전용. |

- CookiieBrain/solar 의존 없이 단독 설치·사용 가능.  
- `deep_engine` 없으면 스텁 → `[QUIET]`. 덕 타이핑으로 결합.  
- 신호 DTO는 **frozen** — 위변조 불가.

---

## 목차

1. [서사적 출처](#1-서사적-출처-본편과-독립-엔진) — 이 패키지가 나온 곳  
2. [서사 구조](#2-서사-구조) — 지하의 목소리, 측정만·행동 없음  
3. [엔지니어링 관점](#3-엔지니어링-관점) — 아키텍처, 데이터 흐름, 모듈 책임  
4. [활용성·확장성](#4-활용성확장성) — 쓰는 곳, 넓히는 방향  
5. [추후 확장 개념](#5-추후-확장-개념) — 로드맵  
6. [설치·사용](#6-설치사용) — Install, Quick Start, API  
7. [통합·제약·출처](#7-통합-제약-출처)
...
- **룰**: `rules.py`만 수정. `world_snapshot` 연동으로 severity/민감도 보정 (덕 타이핑 유지).  
- **다중 신호**: 현재 `List[ConsciousnessSignal]` 구현됨. Pub/Sub·다중 수신자는 호출 측에서 조합.  
- **코어 프로토콜**: `DeepSnapshot` 필드(magnetic_ok, thermal_ok, gravity_ok 등)만 맞추면 어떤 엔진이든 덕 타이핑으로 연결 가능.  
- **독립 재사용**: 다른 시뮬레이터·에이전트 프레임워크에 "거시 진단 레이어"만 끼워 넣어 재사용 가능.
...
**덕 타이핑 코어**  
`magnetic_ok`, `thermal_ok`, `gravity_ok` 만 있으면 동작.
...
- **서사적 출처**: [CookiieBrain](https://github.com/qquartsco-svg/cookiie_brain) `solar/` 천지창조·에덴 로직 안의 **"지하(하데스)의 목소리"** 레이어를 의존성 없이 분리한 독립 패키지.  
